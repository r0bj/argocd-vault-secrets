...
var (
	...
	insecureSsl = kingpin.Flag("insecure-ssl", "Accept/Ignore all server SSL certificates").Envar("AVS_INSECURE_SSL").Bool()
...
func injectVaultDataIntoManifests(manifests []map[string]interface{}, annotationVaultPathKey string) ([]map[string]interface{}, error) {
	...
	vaultClient, err := vaultLogin(string(kubeToken), *kubeAuthMountPath, *vaultRole, *caCert, *insecureSsl)
	if err != nil {
